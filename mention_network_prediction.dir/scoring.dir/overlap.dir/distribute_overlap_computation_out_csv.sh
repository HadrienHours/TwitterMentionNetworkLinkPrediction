...
do
    ...
    then
        ...
        echo "ssh $userName@$mac \"(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate; python compute_overlap_score_from_mention_neighbor_dist_out_csv.py -i1 <(zcat $FILEIN) | gzip > $FILEOUT')\""
        if [ $debug -gt 2 ]
        then
        ...
        then
            echo -e "Following command launched on \e[1m$mac\e[0m on \e[3m$(date)\e[0m"
            echo "compute_overlap_score_from_mention_neighbor_dist_out_csv.py -i1 $FILEIN | gzip > $FILEOUT"
        fi
        ssh $userName@$mac "(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate; python compute_overlap_score_from_mention_neighbor_dist_out_csv.py -i1 <(zcat $FILEIN) | gzip > $FILEOUT')"
    fi
done 
