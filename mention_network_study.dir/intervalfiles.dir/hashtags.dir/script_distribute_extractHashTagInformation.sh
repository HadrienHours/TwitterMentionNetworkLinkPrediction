...
if [ $NARGS -ne $# ]
then
    echo -e "\n\t$(tput bold)Usage $(basename $0):<listfiles><outputdir><login><listmachines>$(tput sgr0)\n"
    exit 1
fi
...
do
    ...
    FILEOUT=$DIROUT/$(basename $filein | sed -re 's/\.[a-z]+$//1')_HASHTAGS_INFOS.tgz

    if [ $debug -gt 1 ]
    then
        echo "ssh $userName@$mac \"(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate;./script_extract_hashtag_infos.sh $FILEIN $FILEOUT')\""
    else
        if [ $debug -gt 0 ]
        then
            echo -e "\n################################"
            ...
            echo -e "################################"
            echo "cmd launched ./script_extract_hashtag_infos.sh $FILEIN $FILEOUT"
        fi
        ssh -n $userName@$mac "(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate;./script_extract_hashtag_infos.sh $FILEIN $FILEOUT')"
        if [  $renice -gt 0 ]
        then
