...
if [ $NARGS -ne $# ]
then
    echo -e "\n\t$(tput bold)Usage $(basename $0):<listdates><inputdir><outputdir><login><listmachines>$(tput sgr0)\n"
    exit 1
fi
...
do
    ...
    do
        ...
        FILEOUT=$DIROUT/$(basename $l | sed -re 's/\.[a-z]+//1')_period_${date1}-${date2}.tgz

        if [ $debug -gt 1 ]
        then
            echo "ssh $userName@$mac \"(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate;python extractHashInfoPerPeriod.py -i <(zcat $FILEIN) -d1 $date1 -d2 $date2 | gzip > $FILEOUT')\""
        else
            if [ $debug -gt 0 ]
            then
                echo -e "\n################################"
            ...
            ssh -n $userName@$mac "(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate;python extractHashInfoPerPeriod.py -i <(zcat $FILEIN) -d1 $date1 -d2 $date2 | gzip > $FILEOUT')"
            if [  $renice -gt 0 ]
            then
