...
#    plt.figure(figsize=(6, 8))
    ...
    #ax3 = ax_main.twinx()
    ax_main.plot(range(len(listoverlaps1)),listoverlap_mention,'r--',lw=1.5,marker='*',markersize=10,label='Overlap mention neighbors')
    ax_main.plot(range(len(listoverlaps1)),listoverlap_random,'b--',lw=1.5,marker='.',markersize=12,label='Overlap random nodes')
    #ax_main.set_ylabel('Average closeness for users',fontsize=24)
    if log:
        ax_main.set_yscale('log')
    #ax_main.yaxis.label.set_color('r')
    #plt.legend(fontsize=12,loc = 'center left')
    ax_main.legend(fontsize=12,loc = 'center left')
