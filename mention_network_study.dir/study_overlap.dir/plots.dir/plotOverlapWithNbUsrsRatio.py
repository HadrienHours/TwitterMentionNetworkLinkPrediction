...
def plotSimNUsr(fileoverlap,fileout,tit,log):
    ...
    listratios = [n/r for n,r in zip(listoverlap_mention,listoverlap_random)]
    ...
    for t1 in ax1.get_yticklabels():
            t1.set_color('b')
    ax2 = ax1.twinx()
    ax2.plot(range(len(listratios)),listratios,'r--',lw=1.5,marker='.',markersize=10)
    #ax2.plot(range(len(listoverlaps1)),listoverlap_random,'g--',lw=1.5,marker='*',markersize=10,label='Overlap random nodes')
    ax2.set_ylabel('Overlap N / Overlap R',fontsize=24)
    ax2.yaxis.label.set_color('r')
    if log:
        ax2.set_yscale('log')
#    plt.legend(fontsize=12,loc = 'lower rightt')
    for t1 in ax2.get_yticklabels():
            t1.set_color('r')
    plt.grid()
    if tit:
        plt.title('Comparison average ratio overlap\nNeighbors nodes/Random nodes\nEvolving network',fontsize=28,fontweight='bold')
    plt.tight_layout()
    plt.draw()
    plt.savefig(fileout)
    plt.close()
