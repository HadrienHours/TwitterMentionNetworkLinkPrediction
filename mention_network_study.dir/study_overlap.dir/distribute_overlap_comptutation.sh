...
do
    ...
    then
        ...
        echo "ssh $userName@$mac \"(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate; python compute_node_link_overlap_from_list.py -i1 <(zcat $FILETRN) -i2 <(zcat $FILETST) -i3 <(zcat $FILEUSR) -on $FILEOUTNB -or $FILEOUTRN -ov $VECTFILE')\""
        if [ $debug -gt 2 ]
        then
        ...
        ssh $userName@$mac "(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate; python compute_node_link_overlap_from_list.py -i1 <(zcat $FILETRN) -i2 <(zcat $FILETST) -i3 <(zcat $FILEUSR) -on $FILEOUTNB -or $FILEOUTRN -ov $VECTFILE')"
    fi
done 
