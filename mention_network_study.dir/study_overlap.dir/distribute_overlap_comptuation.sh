...
if [ $# -ne $NARGS ]
then
    echo -e "\n\t\e[1mUsage $(basename $0):<dirinputtraining_4weeks><dirinputtesting_2weeks><dir_users><dirout><listperiods><login><listmachines>\e[0m\n"
...
do
    ...
    then
        ...
        echo "ssh $userName@$mac \"(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate; python compute_node_link_overlap_from_list_version_20160531.py -i1 $FILETRN -i2 $FILETST -i3 $FILEUSR -on $FILEOUTNB -or $FILEOUTRN -ov $VECTFILE')\""
        if [ $debug -gt 1 ]
        then
    ...
    if [ $debug -lt 3 ]
    then
        ssh $userName@$mac "(cd $CWD && screen -d -m bash -c 'source /datastore/complexnet/test_hours/$env/bin/activate; python compute_node_link_overlap_from_list_version_20160531.py -i1 $FILETRN -i2 $FILETST -i3 $FILEUSR -on $FILEOUTNB -or $FILEOUTRN -ov $VECTFILE')"
    fi
